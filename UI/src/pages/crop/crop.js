...
import {rice} from '../../components/img/crop/rice.JPG'
...
const Crop = () => {
  ...
  const [prediction_text, setPredictionText] = useState("");
    ...
    try {
      fetch(url, {
        ...
        .then((data) => {
          ...
          setPrediction(main_data["prediction"].toUpperCase());
          setPredictionText(main_data["prediction_text"].toUpperCase());
    ...
    <>
      ...
      <section className="">
        ...
        <div>
          ...
          {prediction !== "" ? (
            <div className="result grid place-items-center my-14 text-center ">
              <span className="headerBox">
              <p className="font-bold heder">RECOMMENDED CROP: </p>
              <span>Recommeded crop according to the submitted parameter is</span>
              </span>
              <p className="my-4 cropres">{prediction_text}</p>
              <img
               src={require('../../components/img/crop/'+prediction.toLowerCase()+'.JPG')}
               className="cropimage"
               alt=""/>
            </div>
          ) : (
